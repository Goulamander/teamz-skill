...
import { get_gdrive_content, edit_portal_content, add_my_content, set_selected_microsites_contents, reset_selected_microsites_contents, get_library_images, send_recommended_content, reset_recommended_error } from '../../../actions/contentPortal'
...
export const AllContents = (props) => {
  ...
  useEffect(() => {
    ...
    processAllDriveData(props.gDriveContent)
  }, [props.gDriveContent])

  useEffect(() => {
    if(!!props.contentTagSerachinput === true && !!props.whenToShareSerachInput === true) {
      let filterContent = props.gDriveContent.filter((card) => {
      ...
      processAllDriveData(filterContent)
  ...
  useEffect(() => {
    ...
    if(!!searchTxt === false) {
      ...
      if(filteredData.length > 0) {
        processAllDriveData(filteredData)
      } else {
        processAllDriveData([])
      }
    }
  ...
  const processAllDriveData = (data) => {
  ...
  const handleAllContentsActions = (key, content, index) => {
    switch(key) {
      case "addToMyContent": 
  ...
  return (
    <Fragment>
      <AllContentsComponent 
        AllContents={allContentsData} 
        _handleAllContentsActions={handleAllContentsActions} 
...
const AllContentsComponent = ({
    customerName,
    AllContents,
    _handleAllContentsActions,
    _editContent,
    ...
    let AllContentsData = viewAllCards ? AllContents : AllContents.slice(0,6);
    ...
    return (
      <div className="events-box">
        <Fragment>
          <h4 className="common-head">{`All contents - ${AllContents.length} contents`} <a className="view-btn float-right" href="javascript:void(0)" onClick={()=>toggleViewAllCards()}>{ viewAllCards ? 'View less' : 'View all' }</a></h4>
                  ...
                  <Row>
                    ...
                    <Col xs={6} sm={4} md={3} className="row-view">
                      ...
                      <div className="description">
                        ...
                        <p className="m-0">Great for one micro-demo or personalized video plus three contents</p>
                      </div>
                      <div className="select-style">
      ...
      <div className="card course-single-group">
        ...
        <div className="card-body p-0 pt-3">
            ...
            <div className="course-company-logo">
              <img src={cLogo} height="20" width="40" />
            </div>
                ...
                <Can
                  ...
                  yes={(attr) => (
                    <div className="d-flex justify-content-end">
                      <CustomActionBox listData={contentActionControls} showIcon={true} handleClick={(key) => _handleAllContentsActions(key, contents, index)} />
                    </div>  
                  )}
                  no={() => (
                    <div className="d-flex justify-content-end">
                      <CustomActionBox listData={contentActionWthRecControls} showIcon={true} handleClick={(key) => _handleAllContentsActions(key, contents, index)} />
                    </div>
                  )}
                />
              </div>
