...
import { edit_portal_content, add_my_content, set_selected_microsites_contents, reset_selected_microsites_contents, get_library_images, get_recommended_content } from '../../../actions/contentPortal'
...
import { contentActionWthRecControls } from '../../../constants/appConstants'
...
export const RecommendedContentsForYou = (props) => {
  let { addMyCntError } = props;
  const [recommendedContentData, SetRecommendedContentData] = useState([]);
  ...
  useEffect(() => {
    setCustomerName();
    props._get_recommended_content();
    // props._get_library_images();
  }, []);

  useEffect(() => {
    if(props.recommendedContents.length)
    SetRecommendedContentData(props.recommendedContents)
  }, [props.recommendedContents])

  useEffect(() => {
    let searchTxt = props.globalContentSearchInput;
    if(!!searchTxt === false) {
      if(props.recommendedContents.length > 0) {
        SetRecommendedContentData(props.recommendedContents)
      } else {
        SetRecommendedContentData([])
      }
    } else {
      let filteredData = props.recommendedContents.filter(data => {
        if(data.doc_name != null)
        return data.doc_name.toLowerCase().indexOf(searchTxt.toLowerCase()) !== -1
      })

      if(filteredData.length > 0) {
        SetRecommendedContentData(filteredData)
      } else {
        SetRecommendedContentData([])
      }
    }
  ...
  useEffect(() => {
    if(!!props.contentTagSerachinput === true && !!props.whenToShareSerachInput === true) {
      let filterContent = props.recommendedContents.filter((card) => {
      ...
      SetRecommendedContentData(filterContent)
  ...
  const handlerecommededContentsActions = (key, content, index) => {
    switch(key) {
      case "addToMyContent": 
  ...
  return (
    <Fragment>
      <RecommendedContentsComponent 
        RecommendedContents={recommendedContentData} 
        _handlerecommendedContentsActions={handlerecommededContentsActions} 
...
const RecommendedContentsComponent = ({
    customerName,
    RecommendedContents,
    _handlerecommendedContentsActions,
    _editContent,
    ...
    let RecommendedContentsData = viewAllCards ? RecommendedContents : RecommendedContents.slice(0,6);
    ...
    return (
      <div className="events-box">
        <Fragment>
          <h4 className="common-head">{`Recommended contents - ${RecommendedContents.length} contents`} <a className="view-btn float-right" href="javascript:void(0)" onClick={()=>toggleViewAllCards()}>{ viewAllCards ? 'View less' : 'View all' }</a></h4>
          <h4 className="common-head"></h4>
          {RecommendedContentsData.length == 0 && 
            <Col>
          ...
          <div className="events-card">
            <Row>
              {RecommendedContentsData.map((contents, i) => {
                // static ampere logo for customer courses
                  return (
                    <ContentCard key={i}
                      contents = {contents}
                      openEditContentsModal= {openEditContentsModal}
                      _handlerecommendedContentsActions = {_handlerecommendedContentsActions}
                      customerName = {customerName}
        ...
        <div className="card-body p-0 pt-3">
            ...
            <div className="d-flex mt-3">
              ...
              <div className="d-flex justify-content-end ml-auto">
                  <CustomActionBox listData={contentActionWthRecControls} showIcon={true} handleClick={(key) => _handlerecommendedContentsActions(key, contents, index)} />
              </div>
            </div>
        </div>
      </div>
...
bindActionCreators(
  {
    _get_recommended_content            :   get_recommended_content,
